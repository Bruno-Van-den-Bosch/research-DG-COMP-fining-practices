...
import pandas as pd
import pickle
from compose_dataset import predictions, predictNewFine, showDecisionTree, overviewSpreadsheet
...
def NewModelCreation():
    ...
    if event == 'Submit':
        try:
            ...
            models, variable_names, main_data_dict = predictions(location_data, main_folder, location_variable, filter_value=filter_value, test_size=test_size, random_state=random_state, file_output_folder=output_folder, extra_var_year=extra_var_year, extra_var_sales=extra_var_sales, max_depth=max_depth)  # filter value needs to be value with capital  begin letter (from dataframe)
            output_values = (models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
        except:
...
def loadLastModel():
    ...
    try:
        ...
        if event == 'Submit':
            ...
            first_return = pickle.load(fileObj)  #  models, variable_names, extra_var_year, extra_var_sales, main_data_dict
            fileObj.close()
    ...
    return first_return  # models, variable_names, extra_var_year, extra_var_sales, main_data_dict
...
def welcomeScreen():
    ...
    if event == 'new':
        try:
            first_return = NewModelCreation()  #models, variable_names, extra_var_year, extra_var_sales, main_data_dict
        except:
...
def finePredictionScreen(models, variable_names, extra_var_year, extra_var_sales, main_data_dict):
    '''
...
def saveModelObject(models, variable_names, extra_var_year, extra_var_sales, main_data_dict):
    '''
    Saves the model
    '''
    ...
    if event == 'Submit':
        ...
        save_object = (models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
...
# screen to generate the spreadsheet
def existingCasesCheck(models, variable_names, extra_var_year, extra_var_sales, main_data_dict):
    '''
    shows data on existing cases
    '''
    sg.theme('Light Blue 1')
    layout = [[sg.Text('Give the spreadsheet file a name (name must be given): ')],
                [sg.InputText(default_text="My_model_overview")],
        [sg.Text('Give a location to save the spreadsheet: ')],
        [sg.InputText(), sg.FolderBrowse()],
        [sg.Submit()]]
    window = sg.Window('Save spreadsheet: enter name', layout)
    event, values = window.read()
    window.close()
    if event == 'Submit':
        model_name = values[0] + '.xlsx'
        full_dir = os.path.join(values[1])
        if not os.path.exists(full_dir):
            os.makedirs(full_dir)
        save_location = os.path.join(values[1], model_name)
        sg.theme('Light Blue 1')
        sg.popup_quick_message('Spreadsheet is created, please wait a moment')
        overviewSpreadsheet(main_data_dict, models, variable_names, extra_var_year, extra_var_sales, target_file=save_location)
        out = True
    else:
        out = False
    return out
...
def modelLoadedScreen(models):
    ...
    layout.append([sg.Text('Do you want to explore the current model? hit explore otherwise hit exit.')])
    ...
    layout.append([sg.Text('If you want the overview of the fines within the model in a spreadsheet, check the box"')])
    layout.append([sg.Checkbox('Generate xlsx', default=False)])
    layout.append([sg.Button('Explore'), sg.Exit()])
    win_layout = [[sg.Column(layout, scrollable=True, vertical_scroll_only=True, size_subsample_height=2)]]
    window = sg.Window('Model has been created', win_layout)
    event, values = window.read()
    window.close()
    next_step = bool(event == 'Explore')
    # save the model if so asked
    if values[0]:
        next_step = saveModelObject(models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
    if values[1]:
        next_step = existingCasesCheck(models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
    return next_step




def casePredictionsLoop(models, variable_names, extra_var_year, extra_var_sales, main_data_dict):
    '''
    loops for as long as user submits new cases
    :return:
    '''
    # start fine predicition screen
    finepredictions = finePredictionScreen(models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
    ...
    while finepredictions:
        if finepredictions != False:
            ...
            layout = [[sg.Text('The prediction of the current model for those variables is: ')],[sg.Text('(interpret with caution)')],
                      ...
                      [sg.Text('Deterrence amount: ' + str(finepredictions[4]))],
            ...
            for key, value in models.items():
                ...
                layout.append([sg.Text('Fine as nominal amount for ' + key + ' with leniency - ' + str(value["total_fine"][1]))])
            layout.append([sg.Text('Do you want to predict another new fine based on the variables of the case? click submit')])
            layout.append([sg.Text('Do you want to return to model overview? click "Model"')])
            layout.append([sg.Submit(), sg.Button('Model'), sg.Button('Exit')])
...
def modelExplore(models, variable_names, extra_var_year, extra_var_sales, main_data_dict):
    '''
    Lets you explore the currently loaded model
    '''
    sg.theme('Light Blue 1')
    layout = [[sg.Text('EC fine prediction model')],
              [sg.Text('The current ')],
              [sg.Text('The models accuracy on the fine is currently (as R2 on test): ')]]
    for key, value in models.items():
        layout.append([sg.Text('fine before leniency in percentage for ' + key +' - ' + str(value["fine_before_leniency"][1]))])
        layout.append([sg.Text('fine as nominal amount for ' + key +' with leniency - ' + str(value["total_fine"][1]))])
        layout.append([sg.Text('interim accuracies for ' + key + ':')])
        layout.append([sg.Text( "   Base Amount: "+str(value["Base_amount"][1]))])
        layout.append([sg.Text("    Additional Amount: " + str(value["Additional_amount"][1]))])
        layout.append([sg.Text("    Aggravating Amount: " + str(value["Aggravating_amount"][1]))])
        layout.append([sg.Text("    Mitigating Amount: " + str(value["Mitigating_amount"][1]))])
        layout.append([sg.Text("    Deterrence Amount: " + str(value["Deterrence_amount"][1]))])
        layout.append([])
    layout.append([sg.Text(str("Do you want to explore existing fines (in spreadsheet) or predict a new one?"))])
    layout.append([sg.Button('Explore'), sg.Button('Predict case'), sg.Exit()])
    win_layout = [[sg.Column(layout, scrollable=True, vertical_scroll_only=True, size_subsample_height=2)]]
    window = sg.Window('Model explorer', win_layout)
    event, values = window.read()
    window.close()
    if event == sg.WIN_CLOSED or event == 'Exit':
        return False
    elif event == 'Explore':
        return existingCasesCheck(models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
    elif event == 'Predict case':
        return casePredictionsLoop(models, variable_names, extra_var_year, extra_var_sales, main_data_dict)
...
if first_return:
    models, variable_names, extra_var_year, extra_var_sales, main_data_dict = first_return
    ...
    if modelLoadedScreen(models):
        i = 0
        while modelExplore(models, variable_names, extra_var_year, extra_var_sales, main_data_dict):
            print("Restarting model explore")
        # start loop
