...
class Party:  # party to contain all relevant info to be used to predict
    ...
    def calculateFinePercentage(self):
        ...
        '''
        self.fine_perc_before = (((self.base_amount * self.duration) + self.additional_amount) * (1+((self.aggravating_amount) - (self.mitigating_amount)))) * (1+self.deterrence_amount)
    ...
    def createParty(self, name,  nominal_fine, base_amount, additional_amount, duration, \
                    ...
                    deterrence_amount, decreases_after_fine, ability_to_pay, commissioner, open_category="same"):
        '''
        ...
        self.deterrence_amount = float(deterrence_amount)
...
def readInData(location: str, decision_name=None, case_number=None, party_name=None, commissioner=None, nominal_fine=None,
               ...
               deterrence_amount=None, decreases_after_fine=None, ability_to_pay=None, year=None, open_category=None, sector=None):
    '''
    ...
    for index, row in dataframe.iterrows():
        ...
        party.createParty(row[party_name], row[nominal_fine], row[base_amount], row[additional_amount], row[duration],
                          row[mitigating_amount], row[aggravating_amount], row[deterrence_amount], row[decreases_after_fine], row[ability_to_pay], row[commissioner], row[open_category])
...
def readVariables(location, name=None, regex=None, party=None, add_before=None, exclusion_check=None, distance_party=None):
    ...
    for index, row in dataframe.iterrows():
        ...
        if variables[row[name]].exclusion_check in ('NA', '', ' ', 'Nan', 'NaN', None):
...
def createMainDataFrame(decisions, variables):
    ...
    main_data_dict["Deterrence_amount"] = []
    ...
    for decision in decisions.values():
        for party in decision.parties.values():
            ...
            main_data_dict["Deterrence_amount"].append(float(party.deterrence_amount))
...
def predictFines(DataFrame, variable_names, test_size=0.15, random_state=1, max_depth=None):
    ...
    all_models = {}
    for amount in ("Base_amount", "Additional_amount", "Aggravating_amount", "Mitigating_amount", "Deterrence_amount"):
    ...
    deterrence_amount = all_models["Deterrence_amount"][2]
    ...
    all_models["fine_before_leniency"] =  [None, None, (((base_amount * duration) + additional_amount) * (
                1 + ((aggravating_amount) - (mitigating_amount)))) * (1 + deterrence_amount), all_models["Base_amount"][3]]
...
def predictions(location_data: str, main_folder:str, location_variable:str, name='variable_name', regex='regex', party='party', add_before='add_before',
               ...
               deterrence_amount='deterrence_amount', decreases_after_fine='decreases_after_fine', ability_to_pay='ability_to_pay', filter_value="Commissioner", random_state=12,
    ...
    return models, variable_names, main_data_dict
...
def predictNewFine(models, commissioner, variables, duration, post_fine_reductions, sales=1):  # sales and years are in the variable list
    ...
    fine.deterrence_amount = models[commissioner]["Deterrence_amount"][0].predict(variables).tolist()[0] # addded .tolist()[0]
    ...
    return fine.base_amount, fine.additional_amount, fine.aggravating_amount, fine.mitigating_amount, fine.deterrence_amount, fine.fine_perc_before, fine.fine_perc_final, fine.nominal_fine


def overviewSpreadsheet(main_data_dict, models, variable_names, extra_var_year, extra_var_sales, target_file= "overview_model.xlsx"):
    '''
    make overview excel and predict all fines in case of each filter value (extra column)
    '''
    overview_dict = main_data_dict.copy()
    if extra_var_year:
        variable_names.append('Year')
    if extra_var_sales:
        variable_names.append('Sales')
    for filter_val in models.keys():
        overview_dict[str("predicted_Base_amount_"+filter_val)] = []
        overview_dict[str("predicted_Additional_amount_"+filter_val)] = []
        overview_dict[str("predicted_Aggravating_amount_"+filter_val)] = []
        overview_dict[str("predicted_Mitigating_amount_"+filter_val)] = []
        overview_dict[str("predicted_Deterrence_amount_"+filter_val)] = []
        overview_dict[str("predicted_Fine_before_Leniency_"+filter_val)] = []
        overview_dict[str("predicted_Total_fine_percentage_"+filter_val)] = []
        overview_dict[str("predicted_Nominal_fine_"+filter_val)] = []
        for i, party_name in enumerate(main_data_dict['Party']):
            variables = {}
            for var in variable_names:
                variables[var] = [main_data_dict[var][i]]
                variables = pd.DataFrame(variables)
            base_amount, additional_amount, aggravating_amount, mitigating_amount, deterrence_amount, fine_perc_before, fine_perc_final, nominal_fine = \
                predictNewFine(models, filter_val, variables, main_data_dict["Duration"][i], main_data_dict["Additional_decreases"][i], main_data_dict["Sales"][i])
            overview_dict[str("predicted_Base_amount_"+filter_val)].append(float(base_amount))
            overview_dict[str("predicted_Additional_amount_"+filter_val)].append(float(additional_amount))
            overview_dict[str("predicted_Aggravating_amount_"+filter_val)].append(float(aggravating_amount))
            overview_dict[str("predicted_Mitigating_amount_"+filter_val)].append(float(mitigating_amount))
            overview_dict[str("predicted_Deterrence_amount_"+filter_val)].append(float(deterrence_amount))
            overview_dict[str("predicted_Fine_before_Leniency_"+filter_val)].append(float(fine_perc_before))
            overview_dict[str("predicted_Total_fine_percentage_"+filter_val)].append(float(fine_perc_final))
            overview_dict[str("predicted_Nominal_fine_"+filter_val)].append(float(nominal_fine))
    overview_data = pd.DataFrame(overview_dict)
    overview_data.to_excel(target_file,sheet_name='fines and predictions')
